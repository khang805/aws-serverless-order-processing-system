...
#### merge=lfs (Conflict Resolution):
Instructs Git to use LFS logic for merge conflicts, picking one version over another based on the file's unique ID rather than attempting a text merge.

#### -text (The Data Type):
Explicitly declares the file is not plain text, preventing Git from attempting to format line endings (like Windows CRLF to Linux LF), which would corrupt the binary file.
